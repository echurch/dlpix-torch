...
global_n_epochs = 50
global_batch_size = hvd.size()*128  ## Can be at least 32, but need this many files to pick evts from in DataLoader
vox = 3 # int divisor of 1500 and 1500 and 3000. Cubic voxel edge size in mm.
...
learning_rate = 0.0005 # 0.010
#learning_rate = 0.03888000033795834/hvd.size()
...
modelname = 'model-fulldnn-next1ton-3mm-lr0005-gpu96-f18.pkl'
historyname = 'history-fulldnn-next1ton-3mm-lr0005-gpu96-f18.csv'
scorename = 'scoreeval-fulldnn-next1ton-3mm-lr0005-gpu96-f18.csv'
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.initialconv = scn.SubmanifoldConvolution(dimension, nPlanes, 32, 18, False)
        self.residual = scn.Identity()
        self.add = scn.AddTable()
        self.sparsebl1 = scn.Sequential().add(
            scn.SubmanifoldConvolution(dimension, 32, 32, 10, False)).add(
            scn.BatchNormLeakyReLU(32)).add(
            scn.SubmanifoldConvolution(dimension, 32, 32, 10, False)).add(
            scn.BatchNormalization(32))
        self.sparsebl2 = scn.Sequential().add(
            scn.SubmanifoldConvolution(dimension, 64, 64, 10, False)).add(
            scn.BatchNormLeakyReLU(64)).add(
            scn.SubmanifoldConvolution(dimension, 64, 64, 10, False)).add(
            scn.BatchNormalization(64))
        self.sparsebl3 = scn.Sequential().add(
            scn.SubmanifoldConvolution(dimension, 96, 96, 10, False)).add(
        ...
        self.downsample1 = scn.Sequential().add(
            scn.Convolution(dimension, 32, 64, [3,3,3], [2,2,2], False)).add(
            scn.BatchNormalization(64)).add(
            scn.LeakyReLU())
        self.downsample2 = scn.Sequential().add(
            scn.Convolution(dimension, 64, 96, [3,3,3], [2,2,2], False)).add(
            scn.BatchNormalization(96)).add(
            scn.LeakyReLU())
        self.downsample3 = scn.Sequential().add(
            scn.Convolution(dimension, 96, 128, [4,4,4], [4,4,4], False)).add(
        ...
        self.downsample5 = scn.Sequential().add(
            scn.Convolution(dimension, 160, 192, [3,3,3], [3,3,3], False)).add(
        ...
        self.linear = nn.Linear(int(2*9*9*9), 2)
...
def load_data():
    ...
    # config files
    main_fname = os.environ['HOME']+'/dlpix-torch/larcvconfig_3mm_train.txt'
...
def prepare_batch(_larcv_interface,mode):
...
def train_fullysupervised():
    ...
    for epoch in range(global_n_epochs):
        ...
        for iteration in range(tr_epoch_size):
            ...
            minibatch_data,minibatch_dims = prepare_batch(_larcv_interface,'train') 
...
def eval_epoch(net,optimizer,_larcv_interface,history_writer,csvfile,metrics,epoch):
    ...
    for iteration in range(te_epoch_size):
        ...
        minibatch_data,minibatch_dims = prepare_batch(_larcv_interface,'test') 
